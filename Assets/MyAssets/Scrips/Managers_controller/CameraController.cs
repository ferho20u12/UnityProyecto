...
public class CameraController : MonoBehaviour
{
    ...
    */
    ...
    void Update()
    {
        ...
        if (Input.GetKey(KeyCode.T))//1 click derecho 2 izquierdo 3 scroll
        {
            _camRotY += _cameraSensitivity;
            _focusPoint.localRotation = Quaternion.Euler(_camRotY, 0, 0);
            _player.localRotation = Quaternion.Euler(0, _camRotX, 0);
        }else
        if (Input.GetKey(KeyCode.G))//1 click derecho 2 izquierdo 3 scroll
        {
            _camRotY -= _cameraSensitivity;
            _focusPoint.localRotation = Quaternion.Euler(_camRotY, 0, 0);
            _player.localRotation = Quaternion.Euler(0, _camRotX, 0);
        }else
        if (Input.GetKey(KeyCode.F))//1 click derecho 2 izquierdo 3 scroll
        {
            _camRotX -= _cameraSensitivity;
            _focusPoint.localRotation = Quaternion.Euler(_camRotY, 0, 0);
            _player.localRotation = Quaternion.Euler(0, _camRotX, 0);
        }
        else if (Input.GetKey(KeyCode.H))//1 click derecho 2 izquierdo 3 scroll
        {
            _camRotX += _cameraSensitivity;
            _focusPoint.localRotation = Quaternion.Euler(_camRotY, 0, 0);
            _player.localRotation = Quaternion.Euler(0, _camRotX, 0);
